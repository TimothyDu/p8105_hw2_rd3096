...
``` r
baker_bake=
  left_join(baker_clean, bake_clean,by="baker")
```

    ## Warning in left_join(baker_clean, bake_clean, by = "baker"): Detected an unexpected many-to-many relationship between `x` and `y`.
...
``` r
bake_combine=
  left_join(baker_bake,result_clean,by="baker") %>% 
  arrange(baker) %>% 
  relocate(baker,last_name)
```

    ## Warning in left_join(baker_bake, result_clean, by = "baker"): Detected an unexpected many-to-many relationship between `x` and `y`.
    ## ℹ Row 1 of `x` matches multiple rows in `y`.
    ## ℹ Row 277 of `y` matches multiple rows in `x`.
    ## ℹ If a many-to-many relationship is expected, set `relationship =
    ##   "many-to-many"` to silence this warning.

exporting the final combine dataset

``` r
write_csv(bake_combine,"./gbb/bake_combines.csv")
```
